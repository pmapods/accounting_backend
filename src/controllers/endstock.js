const { inventory, report_endstock, movement } = require('../models')
...
module.exports = {
  getAllReport: async (req, res) => {
    try {
      ...
      if (!limit) {
        ...
        const findLimit = await report_endstock.findAll()
      ...
      const findInventory = await report_endstock.findAndCountAll({
        where: {
          [Op.and]: [
            {
              [Op.or]: [
  ...
  getDetailReport: async (req, res) => {
    try {
      ...
      const findInventory = await report_endstock.findByPk(id)
      if (findInventory) {
    ...
    try {
      ...
      if (listId !== undefined && listId.length > 0) {
        ...
        for (let i = 0; i < listId.length; i++) {
          const result = await report_endstock.findByPk(listId[i])
          if (result) {
      ...
      try {
        ...
        if (type_upload === 'bulk') {
          ...
          for (let i = 0; i < rowList.length; i++) {
            ...
            const findDoc = await report_endstock.findOne({
              where: {
                [Op.and]: [
                  { plant: rowList[i] },
                  { type: type },
            ...
            if (findDoc) {
              ...
              const createData = await report_endstock.create(data)
    ...
    if (type === 'upload') {
      uploadMaster(req, res, async function (err) {
        try {
          ...
          const findId = await report_endstock.findByPk(id)
          if (findId) {
      ...
      if (date !== undefined && listPlant !== undefined && listPlant.length > 0) {
        ...
        const processPlant = (plant) => {
          return new Promise(async (resolve, reject) => {
            try {
              const files = await report_endstock.findAll({
                where: {
                  [Op.and]: [
                    { plant: plant },
                    { date_report: { [Op.between]: [startDate, endDate] } },
              ...
              const report = await report_endstock.create({
                name: 'output_report_endstocktory',
                path: '',
                type: 'output',
                status: 0 // Processing
              })
                ...
                try {
                  if (code === 0) {
                    ...
                    await report.update({
                      ...
                      status: 2, // Success
                      name: `${plant}_output_report_endstocktory_${result.timestamp}`,
  ...
  mergeInventoryReportsOld: async (req, res) => {
    try {
      ...
      const reports = await report_endstock.findAll({
        where: {
          id: listIds,
      ...
      const mergedReport = await report_endstock.create({
        name: 'consolidated_report_endstocktory',
        path: '',
        type: 'consolidated',
        status: 0 // Processing
      })
  ...
  generateInventoryReportReadData: async (req, res) => {
    try {
      ...
      const files = await report_endstock.findAll({ where: { [Op.or]: dataId } })
      const mb51 = files.find(f => f.type === 'mb51')
      const main = files.find(f => f.type === 'main')
